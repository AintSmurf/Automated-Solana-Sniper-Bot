...
class RequestsUtility:
    ...
    def get(self, endpoint: str, payload=None, headers=None, expected_status_code=200) -> json:
        if not headers:
    ...
    def post(self, endpoint: str, payload=None, headers=None, expected_status_code=200) -> json:
        ...
        try:
            ...
            # 🔥 If 429, trigger progressive cooldown
            if self.rs_status_code == 429:
                retry_after = int(rs_api.headers.get("Retry-After", 5))
                self._set_backoff(retry_after)
                time.sleep(self._backoff_delay)
                return self.post(endpoint, payload, headers, expected_status_code)  # retry once

            self.rs_json = rs_api.json()
            self.assert_status_code()
            self._reset_backoff()  # success resets cooldown
